...
this.vala_recruitment <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		this.m.ID = "trigger.vala_recruitment";
	...
	function isTriggerValid(_settlement)
	{
		this.m.Town = _settlement;
		::logInfo("isTriggerValid");
		if (this.World.getPlayerRoster().getSize() >= this.World.Assets.getBrothersMax()) {
			::logInfo("isTriggerValid getPlayerRoster().getSize()");
			return false;
		}

		if (this.World.Assets.getOrigin().getID() == "scenario.legend_risen_legion") {
			::logInfo("isTriggerValid scenario.legend_risen_legion");
			return false;
		}
...
//		}
		...
		foreach( bro in brothers )
		{
			...
			{
				::logInfo("isTriggerValid background.legend_vala");
				return false;
			}

			if (bro.getBackground().getID() == "background.legend_commander_vala")
			{
				::logInfo("isTriggerValid background.legend_commander_vala");
				return false;
			}
